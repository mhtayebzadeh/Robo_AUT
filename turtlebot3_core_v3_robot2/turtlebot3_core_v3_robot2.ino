/*
 *  modified in 17 may 2019
 *  smh tayebzadeh was added camera_pos_tilt and camera_pos_pan and gripper_pos topic 
 *  mhtayebzadeh@yahoo.com
 */
...
void setup()
{
  ...
  cameraPosPanServo.attach(3);
  gripperServoLeft.attach(11);
  gripperServoRight.attach(10);
  ...
  nh.subscribe(camera_pos_pan_sub);
  nh.subscribe(camera_pos_tilt_sub);
...
/*******************************************************************************
* Callback function for my Servo
*******************************************************************************/

void cameraPosTiltCallback(const std_msgs::Int32& msg)
{ 
  int a = constrain((int)msg.data,  25, 100);
  cameraPosTiltServo.write(a);
  
}

void cameraPosPanCallback(const std_msgs::Int32& msg)
{ 

  cameraPosPanServo.write((int)msg.data);
  
}

void gripperCallback(const std_msgs::Int32& msg)
{ 
  int a = (int)msg.data;
  a = constrain(a,  0, 40);
  int r = (int)(1.5 * (float)a)+120;
  int l = (int)(-1.75 * (float)a)+180;
  
  gripperServoLeft.write(l);
  gripperServoRight.write(r);
